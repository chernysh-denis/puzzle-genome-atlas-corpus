...
- [Confirmed | Corroborated | High] World 1 converts finite rail placement into
  a locked simultaneous schedule for one to four numbered carriages.
- [Confirmed | Corroborated | High] Connectivity is insufficient: the same
  layout must avoid temporal collisions and produce ascending coupling order.
...
- `ACT-028` and `TIM-009` received representation-neutral generalisations; no
  previous genome changed.

## Нові питання

- Which independent layout puzzle also encodes vehicle order through path
  length, allowing the traversal/scheduling core to recur?

## Наступна рекомендована гра

- `Golf Peaks`, scoped to World 1.
- Optimisation criterion: test reusable card-driven movement, elevation and
  exact route completion without another automatic transport layout.
- Expected information gain: separate ordered movement-card consumption from
  ordinary avatar navigation and from Bonfire Peaks carry-conditioned height.
- Backlog impact: retain Chants of Sennaar, Tametsi and Isles of Sea and Sky as
  mechanically independent later candidates.

## Чому саме вона

- Railbound closes the immediate transport-layout test. Golf Peaks changes
  interaction family while retaining unusually strong creator documentation
  and a bounded introductory world suitable for the next Goal unit.
