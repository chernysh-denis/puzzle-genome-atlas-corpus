...
- [Confirmed | Direct | High] Contextual dispatch assigns reusable typed
  followers to autonomous work; carrying begins at visible summed strength and
  ends with base intake (`PKM-003`–`PKM-008`).
- [Confirmed | Direct | High] Pikmin 4 combines a real-time sunset-bounded work
  interval with selectable earlier autosave restoration and alternate replay
  (`PKM-009`–`PKM-012`).

## Нові гени

- [Observation | Corroborated | High] Added `ACT-050`–`ACT-052`, `SYS-066`–
  `SYS-068`, `CON-080`, `CON-081`, `INF-021` and `OBJ-023`; reused eight genes.

## Нові комбінації

- [Observation | Corroborated | High] Registered `COMB-0035` for contextual
  cooperative follower extraction; Pikmin 4 also becomes the third supporter
  of recurring `COMB-0034`.

## Зміни таксономії

- [Observation | Corroborated | High] Змін таксономії немає; typed actor
  permissions and branchable rewind were representation-neutral enough to
  admit follower classes and checkpoint-granularity restoration.

## Нові питання

- Does Patrick's Parabox reuse the Sokoban core strongly enough to create a
  third recurring cross-game combination while isolating recursive topology?
- Should the 36-game boundary trigger a checkpoint before or after that test?

## Наступна рекомендована гра

- [Hypothesis | Corroborated | High] `GAME-0036` — Patrick's Parabox.
- Optimisation criterion: follow a high-reuse population analysis with the
  strongest mechanically independent retained subject.
- Expected information gain: test Sokoban action / objective reuse, recursive
  containment against Gorogoa's nested scene graph and whether fixed occupancy
  survives self-containing spaces.
- Backlog impact: retain The Swapper, Viewfinder and Carto; schedule checkpoint
  036 immediately after `GAME-0036`.

## Sources consulted

- Nintendo official product site, guides, tips and Ask the Developer material.
- Contemporary GamesRadar+, GameSpot and Axios mechanical accounts.
