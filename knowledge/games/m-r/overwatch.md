...
- [Confirmed | Corroborated | High] Поточний Overwatch має звичайний 5v5 Role
  Queue, map vote, Perks і симетричний Control; bounded unit завершується другою
  перемогою в раунді, а не кількістю eliminations (`OW-001`–`OW-010`).

## Нові гени

- [Observation | Corroborated | High] Десять нових меж відокремлюють
  `1-2-2`-чергу, голосування за мапу, legal hero swap, Control percentage,
  overtime і first-to-two settlement від уже наявного hero combat.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0179` — один звичайний Quick Play
  Role Queue Control match від role/map commitments через perks, combat,
  respawn і захоплення до двох переможних раундів.
...
- `FAM-009` — Tactical forecast and counterplay: both teams adapt hero, perk,
  cooldown and ultimate choices against live opponents.
- `FAM-015` — Agent routing and coordination: local sight and team cues must
  become shared target, regroup and objective decisions.
- `FAM-010` — Real-time system pressure: combat, cooldowns, respawn, capture,
  percentage and overtime continue while decisions are made.
- No new family is created from one game.

## Plain-language interpretation

The match begins with commitments before the first shot. The player reserves a
Damage slot, influences the map through a ballot and chooses one unoccupied
hero inside that role. Soldier: 76 offers direct aim and familiar movement, but
his healing field, rockets, ultimate and two perk branches create timing and
positioning choices beyond raw accuracy.

Control turns those fights into a spatial puzzle. Eliminations matter only if
the team converts them into point ownership; leaving the point too early can
give the opponent the contest or overtime needed to reverse the round. A won
submap resets the local point but preserves one round win, so the match asks the
team to solve that coordination problem twice.

## New questions

- How much new vocabulary would Stadium require once its round economy, Armory
  items and hero Powers are bounded independently from ordinary Perks?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0182` — Hearts of Iron IV, the next ordered
  horizon item in `SEARCH_DEMAND_GAME_SELECTION_006`.
- Optimisation criterion: add grand-strategy command, production, logistics,
  fronts and war-state boundaries without importing all DLC or world history.
- Expected gene pressure: multi-scale planning, continuous paused time,
  industrial allocation, division orders and bounded capitulation terminal.
- Anti-bias note: do not generalise Overwatch's match-local symmetric team
  structure into a state-scale asymmetric simulation.

## Next research step

- Integrate `GAME-0182` Hearts of Iron IV as one independently reviewed unit;
  do not start it inside this unit's commit.

## Design lessons

- A live hero shooter can have substantial pre-combat decision structure when
  role reservation, map influence and perk branches are mechanically retained.
- Control separates winning fights from converting fights: only legal point
  ownership advances the round terminal.

## Open questions

- Does a later ordinary 6v6 format preserve enough of this genome to reuse the
  combination, or do flexible role composition and balance tuning change its
