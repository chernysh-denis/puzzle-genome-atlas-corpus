...
- [Confirmed | Direct | High] EA більше не продає оригінальну гру в цифровому
  магазині, тому відтворення обмежено законною фізичною Windows-копією та
  офіційним patch `1.4.0` (`NFSUG-001`–`NFSUG-003`).
- [Observation | Corroborated | High] Перший двоколовий Circuit на Olympic
  Square завершується лише перемогою, що дає `375` Bank на Easy і зберігає
  проходження Race 1 (`NFSUG-005`–`NFSUG-008`).
...
- [Observation | Corroborated | High] `COMB-0215` поєднує керування авто,
  суперників, два кола, перше місце, Bank і збережене проходження події.

## Зміни таксономії

- [Confirmed | Direct | High] Жодну сигнатуру раніше перевіреної гри не
  змінено; повторно використаним генам додано лише нову доказову підтримку.

## Нові питання

- Чи утворює окремий Drag-пакет із примусовим ручним перемиканням достатньо
  інший цикл рішень для майбутньої внутрішньоігрової варіації?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0218` Counter-Strike.
- Optimisation criterion: compare one stable classic round against the newer
  Counter-Strike 2 signature without treating sequel history as one ruleset.
- Expected information gain: isolate classic purchase, economy, round and
  persistent-map boundaries from current loadout and service changes.
- Backlog impact: continue Batch 010 while preserving NFS franchise spacing.

## Чому саме вона

- [Hypothesis | Limited | Medium] The classic Counter-Strike round should
  strongly overlap the tactical shooter corridor while exposing a separately
  bounded economy and legacy-client evidence problem.
