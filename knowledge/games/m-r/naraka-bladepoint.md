...
- [Confirmed | Corroborated | High] `NAR-001`–`NAR-011`: current Easy Solo BOT
  Mode on Wanchu links spawn choice, loot, melee counters, hook traversal,
  Shadow Corruption and one Rebirth to a complete placement/Victory terminal.

## Нові гени

- [Observation | Corroborated | High] Added `ACT-360`, `ACT-361`,
  `SYS-652`–`SYS-659` and `CON-530`–`CON-533`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0200` — spawn-selected melee
  survival through hooks, Focus/Counter, contracting safety and one Rebirth.
...
- Which PEAK daily-island identity provides a reproducible solo ascent packet?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0203` — PEAK.
- Optimisation criterion: test physics climbing, stamina, injury and one daily
  generated summit/rescue terminal after melee battle royale.
- Expected information gain: separate cooperative-rescue-capable climbing from
  hook mobility, combat survival and ordinary route traversal.
- Backlog impact: continue Selection 008 without amendment if one dated island
  identity and explicit rescue terminal remain reproducible.

## Чому саме вона

- [Hypothesis | Limited | Medium] PEAK should be mechanically distant from the
  current combat packet while reusing real-time movement, finite supplies and
  bounded run loss.
