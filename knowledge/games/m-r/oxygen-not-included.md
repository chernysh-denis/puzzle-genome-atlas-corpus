...
- Автономна праця Дублікантів залежить від двох шарів пріоритетів, дозволів,
  навичок і доступності маршруту.
- Життєзабезпечення, виробництво й середовище працюють в одній клітинній
  симуляції маси, тепла та фазових переходів.
- Навчання є не чистим бонусом: воно підвищує індивідуальні очікування моралі.
- Базовий Temporal Tear дає відтворюваний довгий горизонт без завершення сейву.

## Нові гени

- `ACT-144`–`ACT-149`; `SYS-185`–`SYS-194`; `CON-192`–`CON-197`;
  `INF-069`–`INF-071`; `OBJ-060`.

## Нові комбінації

- `COMB-0123` — автономна праця колонії всередині спроєктованого життєзабезпечення.

## Зміни таксономії

- Нових родин не створено. Гра включена до `FAM-008`, `FAM-010`, `FAM-015`
  і `FAM-017`; Dwarf Fortress перевірить, чи потрібне уточнення меж родини.
