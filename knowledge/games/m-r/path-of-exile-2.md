...
- [Confirmed | Corroborated | High] Path of Exile 2 patch 0.5.4f joins
  generated Act 1 areas, Skill/Support composition, connected passive growth,
  item drops and crafting, charged recovery and Softcore reset to the Count
  Geonor threshold (`POE2-001`–`POE2-009`).

## Нові гени

- [Observation | Corroborated | High] Added nineteen bounded genes for Gem
  engraving and socketing, currency and Flask actions, area/loot/build/reset
  resolution, item/Support/crafting constraints and build-information panels.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0160` isolates Gem-supported,
  connected-passive checkpoint combat into an authored act guardian.

## Зміни таксономії

- [Observation | Corroborated | High] No taxonomy migration; four existing
  families and eighteen established generic genes are reused unchanged.

## Нові питання

- Will another action RPG recur the strict Skill/Support Gem plus local
  Softcore-reset subset without Path of Exile 2's currency-item economy?

## Наступна рекомендована гра

- [Hypothesis | Limited | High] Complete
  `SEARCH_DEMAND_GAME_SELECTION_004` before assigning `GAME-0163`.
- Optimisation criterion: close the authorised nine-game horizon without
  silently inventing the next batch.
- Expected information gain: identify the next demand-led games and their
  version-specific analysis questions.
- Backlog impact: records a selection unit as the only valid next step.

## Чому саме вона

- [Hypothesis | Limited | High] `GAME-0162` exhausts the complete current
  selection; a new evidence-backed ranking must precede further IDs.

## Localisation status

- Ukrainian game, gene and combination entries: reviewed in this unit.
- Canonical title remains `Path of Exile 2`; the Ukrainian interface appends
  the descriptive title `Шлях вигнанця 2`.

## Open questions

- Re-check the live patch boundary before any future review-on-touch.
- A later direct-play unit could measure the exact first-act availability of
  individual Gems and currencies without changing the present mechanic types.

## Source notes

- Official pages were checked on 2026-08-27. The 0.5.4f thread was the newest
  public game patch in the official index; later service notices did not alter
  the scoped mechanics.
- Community-wiki records are treated as reproducible secondary evidence, not
  as primary authority. No exact stochastic drop rates are asserted.

## Next recommended action

- Complete `SEARCH_DEMAND_GAME_SELECTION_004` before assigning `GAME-0163`.
