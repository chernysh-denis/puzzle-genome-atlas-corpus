...
- [Confirmed | Direct | High] Current Valve Casual Core Payload supports one
  fresh `pl_upward` match with stock-class control, checkpoint time/spawn
  transitions, respawn waves, cart resupply and a bounded overtime terminal
  (`TF2-001`–`TF2-011`).

## Нові гени

- [Observation | Corroborated | High] Added six genes for spawn-legal duplicate
  class selection, checkpoint transformation, shared team waves, cart resupply,
  class-change legality and the Payload-specific result.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0185` isolates the class/wave/cart/
  checkpoint/supply/overtime interaction that produces one Upward result.

## Зміни таксономії

- [Observation | Corroborated | High] No lifecycle migration or reviewed-game
  signature change; four established generic boundaries gain TF2 evidence.

## Нові питання

- Which later class shooter preserves shared respawn-wave timing while replacing
  the asymmetric route cart or attacker-only moving supply field?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0188` — FINAL FANTASY XIV Online.
- Optimisation criterion: continue the recorded demand-led Goal in exact order.
- Expected information gain: replace a short team route contest with a bounded
  cooperative MMORPG duty and reproducible terminal.
...
- The canonical brand title remains `Team Fortress 2`; the explanatory
  Ukrainian title is presentation-only.

## Open questions

- Recheck the public Steam build, latest Valve update and Casual Upward
  availability before later review-on-touch; keep unlock-specific mechanics out
  unless a separate loadout scope is authorised.
