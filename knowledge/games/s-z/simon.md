...
- New game: `GAME-0067` Simon.
- New genes: `ACT-076`, `SYS-107`, `SYS-108`, `INF-033`, `TIM-012`.
...
- Поточна одиночна спроба Simon починається з одного сигналу, після кожного
  точного відтворення повторює весь префікс і додає один випадковий сигнал.
- Перша неправильна кнопка завершує спробу; рахунок дорівнює найбільшій
  повністю відтвореній довжині, а не кількості всіх натискань.
- Ціль показується послідовно й не лишається видимим списком під час відповіді;
  звук можна вимкнути без зміни правила.

## Нові гени

- `ACT-076` — відтворити показану впорядковану послідовність сигналів.
- `SYS-107` — повтор збереженої послідовності з додаванням одного сигналу.
- `SYS-108` — позиційна перевірка відповіді до першої помилки.
- `INF-033` — тимчасовий послідовний показ цільових сигналів.
- `TIM-012` — чергування автоматичного показу та відтворення гравцем.
...
- Нових типів не додано. П'ять нових генів уточнюють межі всередині чинних
  Action, System Behaviour, Information і Time; Constraint лишається порожнім.
