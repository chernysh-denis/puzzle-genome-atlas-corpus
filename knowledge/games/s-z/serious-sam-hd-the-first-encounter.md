...
- [Confirmed | Direct | High] Поточний Windows-продукт, 2009 HD-remейк,
  окремі single-player/co-op поверхні й публічну збірку зафіксовано в
  `SSHD-001`–`SSHD-003`.
- [Observation | Corroborated | High] Вхід до Sand Canyon, авторські випуски
  груп ворогів, обов'язкове очищення кімнати й перехід до Tomb of Ramses III
  зафіксовано в `SSHD-004`–`SSHD-007` і `SSHD-011`.
- [Observation | Corroborated | Medium] Типізовані скінченні боєприпаси поруч
  із необмеженою резервною зброєю зафіксовано в `SSHD-008`.

## Нові гени

- [Observation | Corroborated | High] `SYS-749` відділяє авторський випуск
  скінченної групи за порогом пересування, поразкою цілі або збиранням предмета.
- [Observation | Corroborated | High] `CON-578` відділяє сумісний типізований
  запас зброї від магазинів, перезаряджання та класових слотів.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0235` — авторські скінченні групи,
  живий вогонь, сумісні боєприпаси, повне очищення проміжного шлюзу й фіксований
  вихід до наступника.

## Зміни таксономії

- [Observation | Corroborated | High] Змін меж або підписів раніше перевірених
  ігор немає.

## Open questions

- A direct clean-install capture could tighten the exact level-start inventory,
  public executable version, armour absorption and save-menu behaviour without
  changing the bounded signature.
- A later focused comparison could test whether enough independent carriers
  support a shared superclass above `SYS-621` and `SYS-749`; this unit does not
  rewrite the reviewed Left 4 Dead 2 boundary.

## Нові питання

- Чи підтримає ще одна авторська гра `SYS-749` через збирання предмета, не
  додаючи нескінченного породження або адаптивного керівника сутичок?
- Чи зберігається `CON-578` у старішому FPS, де типізований запас витрачається
  без магазинного перезаряджання, але немає необмеженої резервної зброї?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0238` — Max Payne (2001).
- Optimisation criterion: retain direct authored combat and finite weapon
  resources while replacing first-person trigger-swarm routing with a
  third-person chapter and spendable Bullet Time.
- Expected information gain: time-rate manipulation, shoot-dodge commitment,
  painkiller recovery and chapter transition without adaptive or wave combat.
- Backlog impact: continues `SEARCH_DEMAND_GAME_SELECTION_012` in order; no
  reserve or amendment is required.

## Чому саме вона

- [Hypothesis | Limited | Medium] Наступна гра збереже авторський маршрут,
  вогнепальну зброю й скінченні ресурси, але перевірить, чи утворюють кероване
  сповільнення часу, shoot-dodge та painkiller-лікування окрему переносну межу.

## Revision history

- 2026-09-03: added `GAME-0237` with a bounded current Windows Steam Build
  `5820849`, base single-player `Normal` Sand Canyon packet, full evidence
  ledger, transition trace, new transferable trigger/ammunition genes,
  exhaustive lower-ID comparison and strict combination support.
