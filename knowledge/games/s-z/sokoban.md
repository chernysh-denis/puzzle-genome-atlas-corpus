...
- [Confirmed | Corroborated | High] Sokoban separates keeper navigation from a
  push that changes both keeper and crate positions (`SOK-001`, `SOK-005`).
- [Confirmed | Corroborated | High] Push-only geometry permits non-terminal
  states with legal walking but no remaining solution (`SOK-004`).
- [Pattern | Corroborated | Medium] Keeper access and multi-crate deadlock
  structure matter beyond simple distance to goals (`SOK-006`).

## Нові гени

- [Observation | Corroborated | High] `ACT-008`, `ACT-009`, `CON-011`,
  `CON-012` and `CON-013`.
- [Observation | Corroborated | High] `CON-001`, `INF-001`, `OBJ-004` and
  `TIM-002` are reused.

## Нові комбінації

- [Confirmed | Corroborated | High] `COMB-0006` — self-paced navigation and
  push-only reconstruction under non-terminal deadlock risk.

## Зміни таксономії

- [Observation | Corroborated | Medium] Змін таксономії немає. Objective
  reachability loss fits the Constraint type.

## Нові питання

- TODO: test `CON-013` in another family where an irreversible commitment
  destroys success without ending play.
- TODO: compare Sokoban with a pull-enabled transport puzzle to isolate which
  deadlocks survive when `CON-012` changes.
- TODO: test whether keeper walking should be compressed to reachability only
  in a puzzle whose interface directly commands pushes rather than steps.

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0007` — FreeCell.
- Optimisation criterion: test constrained sequencing and temporary storage
  after deterministic spatial transport.
- Expected information gain: expose tableau access order, bounded free-cell
  buffers, suit/rank build constraints, multi-card transfer conventions and
  automatic-versus-manual foundation movement.
- Backlog impact: FreeCell moves from the coverage pool to the immediate task;
  Sokoban leaves the pool after completion.

## Чому саме вона

- [Hypothesis | Limited | Medium] FreeCell adds a fully visible but highly
  ordered state in which temporary empty positions are an explicit reusable
  resource. It tests whether capacity, access order and sequence-building need
  boundaries beyond Sokoban's spatial access and 2048's generic occupancy.
