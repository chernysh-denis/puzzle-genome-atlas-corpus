...
- [Confirmed | Corroborated | High] Base Rebirth couples seed-generated room
  graphs, live clearance, typed pickups, cumulative item effects and terminal
  run reset inside the first Mom route (`ISAAC-001`–`ISAAC-010`).

## Нові гени

- [Observation | Corroborated | High] Added fourteen genes for bomb placement,
  generated floors, room settlement, heart layers, item composition, boss
  sequence, run/reset persistence, typed resource gates, item slots, room/map
  information and the first Mom objective.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0162` isolates the generated
  room-clearance and disposable-item-build route to the first Mom ending.

## Зміни таксономії

- [Observation | Corroborated | High] No taxonomy migration; fourteen generic
  genes and four existing multi-game families are reused unchanged.

## Нові питання

- Which later action roguelike first reuses both local combat-room locking and
  cumulative disposable-run item composition without Isaac's typed pickups?

## Наступна рекомендована гра

- [Hypothesis | Limited | High] `GAME-0165` — Red Dead Redemption 2.
- Optimisation criterion: continue the recorded demand-led horizon in exact order.
- Expected information gain: compare authored open-world traversal, horse state,
  witnesses, law response, camp resources and mission gates against the corpus.
- Backlog impact: third of nine authorised game units.

## Чому саме вона

- [Hypothesis | Limited | High] It is the next immutable subject in
  `SEARCH_DEMAND_GAME_SELECTION_004` and contrasts a disposable generated run
  with a persistent authored story-world route.

## Localisation status

- Ukrainian game, new-gene and combination entries are reviewed in this unit.
- The canonical title remains `The Binding of Isaac: Rebirth`; the Ukrainian
  interface appends the established descriptive translation «Жертвопринесення
  Ісаака: Відродження».

## Open questions

- A later direct-play module could record one generated seed and verify the
  exact hidden-room, item-charge and simultaneous-terminal edge cases.
- Repentance requires a separate versioned analysis because it changes items,
  routes, balance, UI and multiplayer rather than merely adding parameters.

## Source notes

- Official product, DLC, launch and creator materials were checked on
  2026-08-27. Version-aware wiki pages were used only where publisher material
  does not enumerate the base game's room, pickup and heart transitions.
- DLC-labelled facts were excluded. A wiki page covering several versions was
  admitted only where its base-Rebirth branch or explicit version markers
  preserve the scoped rule.

## Next recommended action

- Integrate `GAME-0165` — Red Dead Redemption 2 after the required thirty-second
  stop window.
