...
- [Confirmed | Corroborated | High] `W3-001`–`W3-010`: one current base-game
  White Orchard packet joins dialogue, forensic sensing, preparation, real-time
  griffin combat, trophy hand-in and retained quest completion.

## Нові гени

- [Observation | Corroborated | High] Added `ACT-370`, `SYS-680`, `INF-265`
  and `OBJ-127`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0203` — clue-led preparation into
  trophy-backed monster-contract settlement.
...
- Which current Yu-Gi-Oh! Master Duel solo gate exposes a version-stable deck-
  building and duel terminal without importing the whole live-service pool?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0206` — Yu-Gi-Oh! Master Duel.
- Optimisation criterion: test explicit card-rule sequencing after one
  evidence-led real-time contract.
- Expected information gain: move from embodied search/combat to a fully
  inspectable turn-and-chain rules surface.
- Backlog impact: continue Selection 008 only after a current solo gate and
  card-pool/rules boundary are fixed.

## Чому саме вона

- [Hypothesis | Limited | Medium] Master Duel should add deck construction,
  summon/chain timing and one duel terminal under a bounded solo ruleset.
