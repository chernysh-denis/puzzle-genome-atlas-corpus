...
- [Confirmed | Corroborated | High] Стабільна Valheim 0.221.12 поєднує
  насіннєвий світ, три активні страви, накритий верстак, відпочинок, дві
  Оленячі Трофеї, Ейктюра та монтаж гарантованого трофея в один перевірний
  перший біом (`VLH-001`–`VLH-012`).

## Нові гени

- [Observation | Corroborated | High] Десять нових меж ізолюють активну їжу,
  Вегвізир, укриття/відпочинок, точний виклик Ейктюра, гарантовану здобич,
  сумісність трофейних споруд і розблокування Сили Зреченого.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0195` — шлях від Вегвізира,
  Meadows-підготовки та двох трофеїв через Ейктюра до змонтованого трофея й
  доступної Сили Зреченого.
...
- `FAM-010` — Real-time system pressure: food/rest decay, ecology, stamina,
  combat, death recovery and boss behaviour resolve while the player acts.
- `FAM-013` — Inventory and fixture dependencies: resources, recipes, covered
  Workbench, food slots, altar offering, gravestone and trophy stone interact.
- `FAM-014` — World topology and perspective: a seed determines geography,
  map discovery exposes it and supported construction changes traversability.
- `FAM-017` — Ordered dependency sequencing: wayfinder, shelter, food/loadout,
  two trophies, summon, defeat, recovery and mounting form a gated chain.
- No new family is created from one game.

## Plain-language interpretation

Valheim's first biome is not merely a search for a boss. The seed decides where
resources and the altar exist, while the spawn Vegvisir turns that generated
space into a visible route. Wood and stone become a supported shelter; roof and
cover make the Workbench usable; fire and comfort turn waiting into Rested
capacity. Three different foods form a temporary health-and-stamina build, so
preparation is an expiring configuration rather than a starvation meter.

Deer hunting supplies two matching trophies. Offering both at the marked altar
starts Eikthyr under live stamina, defence and telegraph rules. Death preserves
the world but taxes skills and separates the player from carried equipment,
making gravestone recovery part of the same attempt. Defeat guarantees the
trophy, yet the unit remains unfinished until it is carried back to the central
stones and mounted. That final fixture converts one boss result into retained
Forsaken Power availability.

## New questions

- Which later survival game reuses the authored-wayfinder to generated-target
  boundary but settles through a non-combat fixture instead of permanent power?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0198` — Brawlhalla.
- Optimisation criterion: complete the authorised demand-led Goal with one
  current fixed private 1v1 stock match.
- Expected gene pressure: damage-driven launch force, weapon pickups, stocks,
  aerial recovery, arena edges and final knockout.
- Anti-bias note: do not import Valheim's persistence, production, death
  recovery or trophy settlement into a self-contained arena match.

## Next research step

- Integrate `GAME-0198` — Brawlhalla after the required thirty-second stop window.

## Design lessons

- A survival sandbox can have a bounded terminal when a generated route feeds
  exact fixtures and the final persistent unlock occurs after the boss result.
- Temporary food and rest state matter because the same equipment can face a
  materially different stamina/recovery envelope without a permanent level gate.

## Open questions

- Does the current client display the Eikthyr power as selected immediately
  after first mounting, or merely make it available for selection? The canonical
  terminal deliberately records selectable/available power until direct play
  confirms the exact first-mount presentation.
