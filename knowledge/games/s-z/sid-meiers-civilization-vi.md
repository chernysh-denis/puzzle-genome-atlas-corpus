...
- [Confirmed | Direct | High] The bounded base-game route joins founded cities,
  citizen yields, separate districts, two research trees, government, trade,
  diplomacy, combat and five Spaceport projects (`CIV6-001`–`CIV6-012`).

## Нові гени

- [Confirmed | Direct | High] Forty-two bounded genes isolate city production,
  Civic research, citizens, policies, trade, diplomacy, city/district economy,
  combat, information, Mars objective and multi-command civilization turns.

## Нові комбінації

- [Confirmed | Direct | High] `COMB-0164` — hex empire science race from first
  city to all three base-game Mars modules.
...
- `FAM-005` — Route and network construction: traders create roads and posts
  between expanding cities.
- `FAM-009` — Tactical forecast and counterplay: movement, combat forecasts,
  diplomacy and rival progress shape committed turns.
- `FAM-015` — Agent routing and coordination: multiple military, civilian and
  trader units share constrained hex space.
- `FAM-017` — Ordered dependency sequencing: two trees, district prerequisites
  and five launch projects form the terminal chain.
- No new family is created from one game.

## Plain-language interpretation

Civilization VI turns geography into a long scientific argument. A citizen is
not only population: assigning that person to one farm, mine or district slot
changes the city's next yields. A district is not a menu building: it consumes
a separate map hex, so mountain adjacency for a Campus or flat terrain for a
Spaceport must be protected many turns before use. Technology and Civics then
advance beside each other, while the chosen government constrains which policy
effects can operate together.

The same map is political. A trader creates both recurring income and a road;
units spend turn budgets under terrain, stacking and control rules; a deal, war
or captured city changes future access and rival pressure. End Turn commits the
whole package before the next civilization acts. The selected route ends only
after the Satellite, Moon Landing and three distinct Mars modules have been
produced in legal order.

## New questions

- Which later 4X game reuses parallel research currencies and map-resident
  districts while replacing policy slots or the ordered space terminal?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0167` — Geometry Dash.
- Optimisation criterion: continue the recorded demand-led horizon in exact order.
- Expected information gain: contrast multi-hour empire turns with one-input
  automatic-motion level memory and instant retry.
- Backlog impact: fifth of nine authorised game units; it is not started here.

## Чому саме вона

- [Hypothesis | Limited | High] Geometry Dash is the next immutable subject in
  `SEARCH_DEMAND_GAME_SELECTION_004` and maximises temporal/control contrast.

## Localisation status

- Ukrainian game, all new-gene and combination entries are reviewed in this unit.
- The canonical title remains `Sid Meier’s Civilization VI`; the Ukrainian
  interface appends «Цивілізація Сіда Мейєра VI».

## Open questions

- A future module should test how the launch rules differ under Gathering
  Storm's exoplanet expedition without treating the expansion as a parameter.
- Direct-play capture could verify the selected seeds' exact starting terrain
  and opponent spawn positions; those are deliberately not claimed here.

## Source notes

- Official manual, product, Steam and Rome materials were checked on 2026-08-27.
- Expansion, DLC, city-state and disabled-victory facts were used only to draw
  exclusions and were not unioned into the active genome.

## Next recommended action

- Integrate `GAME-0167` — Geometry Dash after the required thirty-second stop window.

## Evidence limitations

- No fresh complete Mars playthrough was performed; transition confidence comes
  from the official launch manual and official product material.
- The current Steam executable may include patches and expansion-aware menus;
  claims are bounded to the documented launch/base-game rules, not every current
  balance value or UI layout.
- Rival AI choices remain seed- and state-dependent, so the sources establish
  the legal transition system, not one guaranteed turn-by-turn winning script.
