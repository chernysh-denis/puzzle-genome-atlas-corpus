...
- [Confirmed | Direct | High] The basic line is simultaneously a simple route
  and a wall that induces clue-evaluation regions (`WIT-002`–`WIT-006`).
- [Confirmed | Corroborated | High] Completion validates topology and region
  clues together, then indicates violated clues without exposing a solution
  (`WIT-008`–`WIT-010`).

## Нові гени

- [Observation | Corroborated | High] Added `SYS-073`, `CON-087` and `INF-023`;
  reused six existing genes.

## Нові комбінації

- [Observation | Corroborated | High] Registered recurring `COMB-0039` with
  Flow Free for visible self-paced simple-path constraint completion.

## Зміни таксономії

- [Observation | Corroborated | High] Generalised `ACT-016`, `CON-029` and
  `OBJ-006` wording without a merge, split or prior genome rewrite.

## Нові питання

- Do later line-puzzle subjects recur the region-partition constraint without
  sharing The Witness's exact clue encoding?
- Should checkpoint 040 prioritise normalising generic validation systems if a
  second game supports `SYS-073`?

## Наступна рекомендована гра

- [Hypothesis | Corroborated | High] `GAME-0040` — Carto.
- Optimisation criterion: prefer the mechanically independent retained subject
  over another image / rewind game adjacent to Portal and Braid.
- Expected information gain: distinguish authoritative map-piece rearrangement
  from Gorogoa panel composition and test whether topology changes propagate to
  the traversable world.
- Backlog impact: retain Viewfinder; schedule `CHECKPOINT_040` immediately after
  Carto because four games have accumulated since checkpoint 036.

## Sources consulted

- Official PlayStation product page and Jonathan Blow's TIME interview.
- Abel et al.'s formal computational-complexity analysis.
- Contemporary TIME and Ars Technica reviews plus Thinky Games' mechanical
  feedback account.
