...
- [Confirmed | Direct | High] Stable `1.16.100` supports a fresh-save Antonio
  normal Mad Forest route whose success is stage completion at `30:00` before
  the ordinary Reaper cleanup (`VS-001`–`VS-012`).

## Нові гени

- [Confirmed | Direct | High] Fourteen boundaries isolate clock-indexed waves,
  independently ticking weapons, spatial Experience Gems, paused build drafts,
  chest evolution, continuous health and the completion-before-Reaper terminal.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0181` records the strict chain from
  direct crowd shaping through automatic coverage and XP drafts to fixed-time
  stage survival.

## Зміни таксономії

- [Confirmed | Direct | High] No existing gene, combination, family boundary or
  reviewed game signature changes.

## Family classification

- `FAM-009` — Tactical forecast and counterplay: movement forecasts local crowd
  shape and corrects automatic weapon coverage against visible pressure.
- `FAM-010` — Real-time system pressure: waves, weapon clocks, pickups and stage
  time advance together outside explicit choice pauses.
- `FAM-013` — Inventory and fixture dependencies: Hollow Heart and a typed boss
  chest gate the mature Whip's evolved replacement.
- `FAM-017` — Ordered dependency sequencing: enemy defeat, gem collection,
  level thresholds, item maturity and chest eligibility gate later build state.
- No new family is created from one game.

## Plain-language interpretation

Vampire Survivors removes the attack button but not the combat decisions. The
player's movement decides where crowds gather, which side of Antonio's Whip
connects and whether dangerous experience fields can be harvested. Every
collected level then pauses the pressure long enough to choose one persistent
change to a portfolio that will resume acting on its own.

The optional Whip evolution is a dependency test inside that loop, not the
objective. Maximum-level Whip, Hollow Heart and the right chest opportunity can
produce Bloody Tear, but the bounded success test remains simpler and harsher:
keep one continuous health pool above zero until Mad Forest reaches `30:00`.

## New questions

- Which later auto-battler or survival game reuses spatial XP drafting while
  replacing direct avatar movement or the fixed-duration stage terminal?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0184` — War Thunder, the next ordered item
  in `SEARCH_DEMAND_GAME_SELECTION_006`.
- Optimisation criterion: contrast one fixed solo survival stage with a bounded
  current vehicle battle whose crew, damage and objective rules are explicit.
- Expected gene pressure: vehicle selection, direct combined-arms control,
  component damage, tickets/objectives, respawn allowance and match terminal.
- Anti-bias note: do not carry Vampire Survivors' automatic weapon or disposable
  build assumptions into a manually controlled competitive vehicle ruleset.

## Next research step

- Integrate `GAME-0184` War Thunder as one independently reviewed unit; do not
  start it inside this unit's commit.

## Reproducibility notes

1. Verify stable version `1.16.100` / Steam build `24926490`; remain on the
   public branch and disable DLC, co-op, modifiers and mods.
2. Use an empty save slot, select Antonio and normal Mad Forest, and record the
   first controllable `0:00` frame with Whip and no PowerUps.
3. Record one automatic Whip activation, one enemy-to-Experience-Gem transition,
   one Magnet pickup and one paused level-up selection.
4. If the random run permits it, record maximum Whip, Hollow Heart and an
   evolution-capable chest becoming Bloody Tear; absence does not invalidate
   the survival terminal.
5. Stop at early zero health or record stage completion at `30:00`, first Reaper
   appearance and ordinary result settlement without pursuing Reaper defeat.

## Normalised genome

| Type | Active gene IDs | Candidate genes or parameters |
|---|---|---|
| Action | `ACT-008`, `ACT-140` | direct movement and one persistent level-up choice |
| System Behaviour | `SYS-004`, `SYS-299`, `SYS-469`, `SYS-572`–`SYS-579` | random offers, level progression, waves, automatic weapons, XP, build, evolution, health and terminal |
| Constraint | `CON-188`, `CON-485`–`CON-487` | exclusive offer, slot/level eligibility, evolution gates and health/time order |
| Information | `INF-002`, `INF-119`, `INF-235`, `INF-236` | hidden future rewards, visible character/build, local field and survival HUD |
| Objective | `OBJ-107` | reach normal Mad Forest's `30:00` completion alive |
| Time | `TIM-003` | live forced progression interrupted by paused level choices |

## Corpus comparison

- Comparison algorithm: `genome-jaccard-v1`.
- Prior game signatures scanned: `182` (`GAME-0001`–`GAME-0182`).
- Exact genome matches: none.
- Tied near matches: `GAME-0164` — The Binding of Isaac: Rebirth (`6 / 45 = 0.133333`).
- Supported combination subsets: `COMB-0181`.
...
|---|---|---|---|
| `GAME-0164` — The Binding of Isaac: Rebirth | `ACT-008`, `SYS-004`, `SYS-469`, `INF-002`, `INF-119`, `TIM-003` | Both steer a disposable random build with visible health and hidden future rewards. Isaac aims tears, manages typed pickups and generated room/floor routes to Mom; Vampire Survivors delegates attacks to cooldowns, turns kills into spatial XP drafts and survives an authored wave clock to completion | Near, `0.133333` |

### Preserved research notes

- New genes: `SYS-572`–`SYS-579`, `CON-485`–`CON-487`, `INF-235`–`INF-236`
  and `OBJ-107`.
- Reused genes: `ACT-008`, `ACT-140`, `SYS-004`, `SYS-299`, `SYS-469`,
  `CON-188`, `INF-002`, `INF-119` and `TIM-003`.
- Classification result: fourteen new boundaries plus nine established generic
  genes and one verified strict combination.

## Combination status

- `COMB-0181` is a verified strict subset coupling direct crowd shaping,
  cooldown-driven automatic weapons, spatial XP, interrupted build drafts,
  optional chest evolution and the health-before-time survival terminal.
