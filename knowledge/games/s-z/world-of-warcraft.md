...
- Поточний свіжий персонаж проходить повний Exile's Reach до Darkmaul Citadel,
  після чого маршрут веде до пристані Dragon Isles, а не до старого огляду
  столиці.
- Одиниця фіксує US Retail `12.1.0.69587`, Normal realm, Alliance Human Warrior
  і здавання `An End to Beginnings` Калекґосу.

## Нові гени

- `SYS-716`, `CON-562` і `OBJ-137` відокремлюють автодоповнення функцій групи
  у навчальному підземеллі, його межу допуску та повний збережений термінал.

## Нові комбінації

- `COMB-0219` — від покрокового навчання й ланцюга завдань через постійний
  розвиток персонажа до допоміжного підземелля та здавання в новому регіоні.

## Зміни таксономії

- Додано три Active-межі без зміни наявних генів або сигнатур.

## Нові питання

- Чи відрізнить Aion Classic стартове навчання, якщо рання групова структура не
  є обов'язковим термінальним іспитом одного острівного маршруту?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0222` Call of Juarez: Gunslinger.
- Optimisation criterion: contrast the persistent assisted MMORPG tutorial with
  one finite authored score-and-story shooter packet in Batch 010 order.
- Expected information gain: a bounded arcade narration and duel route.
...
- It is the next authorised subject in `SEARCH_DEMAND_GAME_SELECTION_010` and
  separates the three requested MMORPG units with a different decision loop.
...
Exile's Reach is not “all of World of Warcraft”. It is one short, current route
that teaches a new Human Warrior by giving one readable task at a time. The
player talks, travels, fights, uses a required object and keeps the resulting
level, abilities and equipment. Darkmaul Citadel then turns those lessons into
a small live-party test: the player controls only the Warrior, while the client
supplies the functions needed to finish the two-boss dungeon.

The terminal matters because the live route changed. Reaching the old capital
would describe historical onboarding, not the current client. This record ends
only after the same character reaches the Dragon Isles docks and hands the
final quest to Kalecgos. The Housing Skip is explicitly rejected because it
would create the level without reproducing the learning-and-dungeon chain.
