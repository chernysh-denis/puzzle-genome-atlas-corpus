...
- [Confirmed | Direct | High] Офіційна PC-інструкція відокремлює Campaign від
  Multiplayer і Arcade та прямо описує heat, active cooling, health regeneration,
  droid inputs і повернення до останньої контрольної точки (`SWBF2-002`).
- [Observation | Corroborated | High] `The Cleaner` переносить ID10 із
  безпосередньо керованого тіла в супровідну здатність Iden, а потім поєднує
  purge, живий бій, фінальний slice і збережений наступник (`SWBF2-004`–`SWBF2-012`).

## Нові гени

- [Observation | Corroborated | High] `SYS-737`, `SYS-738`, `SYS-739`,
  `CON-571`, `INF-280`, `OBJ-144`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0228`.
...
- Чи повторюється reusable remote-body-to-companion-ability handoff в іншій
  однокористувацькій кампанії без вибору персонажа?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0231` — Fallout 4.
- Optimisation criterion: перейти від фіксованого бойового прологу до одного
  немодифікованого fresh-start route з раннім quest/checkpoint terminal.
- Expected information gain: перевірити відкритосвітове спрямування, V.A.T.S.,
  loot і квестове збереження без об'єднання DLC, settlements чи Commonwealth.
- Backlog impact: продовжити активний Goal, не починаючи `GAME-0231` у цьому
  unit.

## Чому саме вона

- [Hypothesis | Limited | High] Це наступна записана гра Batch 011 і контрольна
  зміна від лінійної кампанійної місії до раннього відкритосвітового маршруту.
