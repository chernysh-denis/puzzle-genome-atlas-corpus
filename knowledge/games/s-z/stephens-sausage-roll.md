...
- [Confirmed | Direct | High] Lateral versus axial fork contact produces
  different two-cell sausage transitions, and grill state belongs to four
  independently identified faces (`SSR-004`–`SSR-006`).
- [Confirmed | Direct | High] Success combines exact-once cooking with return
  to the exact initial player / fork pose (`SSR-008`).

## Нові гени

- [Observation | Corroborated | High] Added `ACT-058`, `SYS-078`, `SYS-079`,
  `CON-090`, `CON-091` and `OBJ-027`; reused eight existing genes.

## Нові комбінації

- [Observation | Corroborated | High] Registered `COMB-0043`; no earlier
  analysed game supports the complete oriented-tool surface-processing set.

## Зміни таксономії

- [Observation | Corroborated | High] Змін таксономії немає; `ACT-009`,
  `CON-011` and `CON-061` gain representation-neutral examples without prior
  signature changes.

## Нові питання

- Does A Good Snowman's terrain-triggered growth reuse the push core while
  requiring a persistent size-state transformation distinct from cooking?
- Should a later sausage-spearing scope add a carried elongated-body action or
  reuse a broader rigid-object carry boundary after more evidence?

## Наступна рекомендована гра

- [Hypothesis | Corroborated | High] `GAME-0044` — A Good Snowman Is Hard to
  Build.
- Optimisation criterion: test whether another push puzzle reuses the shared
  core while replacing surface-state processing with terrain growth and
  ordered assembly.
- Expected information gain: distinguish growth from merge, stacking from
  occupancy and final snowman assembly from generic Sokoban targets.
- Backlog impact: remove A Good Snowman from the pool; retain Snakebird and
  Hexcells Infinite.

## Чому саме вона

- [Hypothesis | Corroborated | High] It is the highest-ranked retained subject
  and supplies a controlled adjacent comparison immediately after the newly
  bounded exact-once surface mechanics, while still avoiding topology / rewind.

## Sources consulted

- Stephen Lavelle's official page and creator-published Steam listing.
- Jason Liu's formal MIT thesis and contemporary PC Gamer, Guardian and The
  Stack mechanical accounts.
- One bounded World 1 mechanics guide and one level-count artefact used only
  for early-scope confirmation.
