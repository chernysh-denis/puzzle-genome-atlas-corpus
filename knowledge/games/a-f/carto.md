...
- [Confirmed | Direct | High] A legal map edit immediately becomes the actual
  traversable world topology (`CAR-007`, `CAR-013`).
- [Confirmed | Corroborated | High] Even the avatar-bearing fragment can move,
  while terrain and occupants remain fragment-relative (`CAR-004`, `CAR-008`).

## Нові гени

- [Observation | Corroborated | High] Added `ACT-056`, `SYS-074`, `CON-088`
  and `OBJ-026`; reused `ACT-008`, `CON-058`, `INF-001` and `TIM-002`.

## Нові комбінації

- [Observation | Corroborated | High] Registered `COMB-0040`; no prior game
  supports its authoritative persistent map-fragment core.

## Зміни таксономії

- [Observation | Corroborated | High] Generalised `CON-058` wording from one
  hex-landscape scoring rule to typed edge compatibility with forbidden or
  penalised mismatch; no lifecycle change.

## Нові питання

- Does Viewfinder instantiate world geometry while preserving a source image,
  or overwrite it through a materially different world-state transition?
- At checkpoint 040, do map / portal / containment topology genes justify a
  broader relation layer without collapsing their different player actions?

## Наступна рекомендована гра

- [Observation | Corroborated | High] `CHECKPOINT_040` — forty-game corpus
  density, topology-boundary and combination-support audit.
- Required scope: all 40 indexed games, 274 active genes and 478 usages; verify
  singleton support, the post-036 topology cluster, recurring combination
  supporters, taxonomy boundaries and the retained Viewfinder candidate.
- Stop condition: no `GAME-0041` selection before the checkpoint decision.

## Sources consulted

- Humble Games / Sunhead Games' PlayStation developer-publisher announcement.
- Contemporary Game Informer, Nintendo Life, Film Stories and Nintendo World
  Report mechanical accounts.
