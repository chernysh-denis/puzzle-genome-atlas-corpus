...
- [Confirmed | Direct | High] Поточний Windows-продукт, керування й відокремлені
  мережеві поверхні зафіксовано в `FC3-001`–`FC3-003`.
- [Observation | Corroborated | High] Камера, камінь, підозра, закрита група
  ворогів, відсутність сигналізації, здоров'я Dennis і зміна контролю над
  аванпостом зафіксовані в `FC3-004`–`FC3-011`.
- [Observation | Corroborated | High] Вхід до будівлі відкриває наступну місію,
  а збережені місцеві сервіси й наступник утворюють термінал `FC3-012`.

## Нові гени

- [Observation | Corroborated | High] `ACT-405`, `ACT-406`, `SYS-747`,
  `SYS-748`, `INF-287` і `OBJ-147` відділяють переносимі дії, переходи,
  інформацію та мету від конкретних назв місії й аванпосту.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0234` — розвідка з мітками,
  відволікання, закрите очищення й збережене перетворення локального вузла.
...
- Чи повторить пізніший аванпост той самий каркас, коли сигналізація справді
  допускає підкріплення, чи це утворює окрему комбінацію тиску?
- Чи збереже інша гра оптичну мітку цілі крізь перешкоду без одночасного
  перетворення очищеної локації на постійний сервісний вузол?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0237` — Serious Sam HD: The First
  Encounter.
- Optimisation criterion: contrast reconnaissance-led local stealth with an
  authored arena whose pressure is driven by explicit combat waves.
- Expected information gain: finite-wave closure, resource placement,
  continuous kiting and level-terminal retention without optical tagging,
  suspicion or world-node conversion.
- Backlog impact: continues `SEARCH_DEMAND_GAME_SELECTION_012` in order; Far
  Cry reserves and optional modules remain deferred.

## Чому саме вона

- [Hypothesis | Limited | Medium] The next fixed campaign level should retain
  direct first-person movement, finite ammunition and live hostile combat while
  sharply replacing stealth, decoys and persistent local control with
  high-density wave scheduling and arena-resource routing.
