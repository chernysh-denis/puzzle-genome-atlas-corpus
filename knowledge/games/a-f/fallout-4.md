...
- [Confirmed | Direct | High] Поточна основна гра для Windows відокремлена від
  GOTY та Anniversary, а офіційні матеріали визначають сім S.P.E.C.I.A.L.
  характеристик і типи збереження (`FO4-001`–`FO4-006`, `FO4-009`).
- [Observation | Corroborated | High] Маршрут через Vault 111 завершується
  лише після керування назовні, активного `Out of Time` і перевіреного
  завантаження (`FO4-007`–`FO4-011`).

## Нові гени

- [Observation | Corroborated | High] `ACT-401`, `SYS-740`, `CON-572`,
  `INF-281`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0229`.

## Зміни таксономії

- [Observation | Corroborated | High] `OBJ-113` отримує другий приклад і
  уточнення необов'язковості escort branch; раніші сигнатури не змінено.

## Нові питання

- Чи повториться fixed shared creation budget без одночасного lifepath, class
  або background commitment?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0232` — PAYDAY 3.
- Optimisation criterion: перейти до одного фіксованого поточного heist від
  briefing/loadout до escape або failure.
- Expected information gain: перевірити stealth/alarm, objective-tool, custody
  та settlement boundaries без union усіх heists і account grind.
- Backlog impact: продовжити Goal, не починаючи `GAME-0232` у цьому unit.

## Чому саме вона

- [Hypothesis | Limited | High] Це наступна записана гра Batch 011 і зміна від
  одиночного open-world прологу до повторюваного cooperative heist ruleset.
