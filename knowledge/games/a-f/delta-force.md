...
- [Confirmed | Direct | High] Поточна мапа Coliseum підтримує звичайний режим
  Attack and Defend із послідовними секторами (`DF-002`–`DF-004`).
- [Observation | Direct | High] Актуальний запис для Windows фіксує Luna на
  боці оборони та явний результат Victory після виснаження атакувальників
  (`DF-008`).

## Нові гени

- [Observation | Corroborated | High] Чотири нові гени відокремлюють перехід
  між секторами, поповнюваний лише для атакувальників запас підкріплень,
  допустиму активну лінію фронту й точну ціль оборони.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0198` фіксує зв'язок між вибором
  оператора, мережею повернення, посекторним захопленням і виснаженням
  підкріплень атакувальників.

## Зміни таксономії

- [Observation | Direct | High] Раніше перевірені сигнатури не змінено;
  `SYS-396` свідомо не розширено за межі симетричного Conquest.

## Нові питання

- Which later attack/defend game advances an ordered front but falsifies the
  attacker-ticket replenishment boundary?

## Наступна рекомендована гра

- [Confirmed | Corroborated | High] `GAME-0201` PAYDAY 2.
- Optimisation criterion: continue the recorded Selection 008 order after one
  complete local game commit.
- Expected information gain: test four-player cooperative heist planning,
  stealth-to-assault escalation and bounded contract settlement.
- Backlog impact: advances the active 198-to-207 Goal by one bounded unit.

## Чому саме вона

- [Confirmed | Corroborated | High] PAYDAY 2 is the next recorded selection;
  starting another title would violate the ordered horizon.
