...
- [Confirmed | Corroborated | High] У `Now You See It` двері не змінюють
  призначення, поки гравець дивиться на них (`ANT-004`, `ANT-005`).
- [Confirmed | Direct | High] Автор навмисно з'єднував просторово окремі
  кімнати нетрадиційними графовими зв'язками (`ANT-002`, `ANT-003`).

## Нові гени

- [Observation | Corroborated | High] `ACT-098` — свідомо відводити або
  утримувати погляд на геометрії, видимість якої змінює правило.
- [Observation | Corroborated | High] `SYS-129` — перепризначати авторську
  кімнату за стабільним локальним порогом після втрати спостереження.
- [Observation | Corroborated | High] `CON-148` — не дозволяти просторову
  заміну, доки поріг залишається видимим.
- [Observation | Corroborated | High] `INF-047` — показувати локальний поріг,
  але приховувати його поточне віддалене призначення.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0097` — відвести погляд, змінити
  приховану суміжність і пройти тим самим порогом до нової кімнати.

## Зміни таксономії

- [Observation | Corroborated | High] Авторське off-screen remapping відділено
  від порталів, періодичного wrap, рекурсивних копій і проєкційної топології.

## Нові питання

- Який production-критерій видимості дверей і коли саме фіксується нове
  призначення?
- Чи повториться `ACT-098` у грі, де погляд змінює стан об'єкта, але не
  просторову суміжність?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] Hyperbolica.
- Optimisation criterion: test a spatial puzzle whose geometry is continuously
  curved rather than discontinuously reconnected off screen.
- Expected information gain: separate authored graph remapping from navigation
  in a globally non-Euclidean but locally continuous metric.
- Backlog impact: move Hyperbolica to `GAME-0098`; retain The Stanley Parable
  as a later choice-conditioned corridor-state candidate.

## Чому саме вона

- [Hypothesis | Limited | Medium] Hyperbolica should reuse direct first-person
  traversal while falsifying Antichamber's hidden doorway remap: its route can
  bend through a non-Euclidean metric without swapping one threshold's target.
