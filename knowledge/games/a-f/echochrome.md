...
- Added one Action, one System Behaviour, one Constraint, one Information and
  one Objective gene.
- Extended five reused genes with Echochrome evidence.
- Added an executable continuous-projection two-echo control.

## Нові факти

- Walker рухається автономно, а гравець керує маршрутом через перспективу.
- Віддалені у 3D кінці стають переходом, якщо збігаються на екрані.
- Розрив, закритий ближчим елементом, тимчасово стає прохідним.
- Контроль довів один суміщений перехід, один прихований розрив, два echoes і
  шість відхилених переходів без зміни світової геометрії.

## Нові гени

- `ACT-095` — обертати камеру з наслідком для правил перспективи.
- `SYS-121` — поточна екранна проєкція визначає топологію проходу.
- `CON-143` — маршрут Walker контролюється камерою, а не прямим рухом.
- `INF-042` — авторитетний екранний контакт або закриття видно наживо.
- `OBJ-049` — зібрати всі цілі непрямо спрямованим Walker.

## Нові комбінації

- `COMB-0092` — керована камерою автономна хода через авторитетну проєкцію.

## Зміни таксономії

- Безперервну перспективу відокремлено від чотирьох станів Fez.
- Видиму суміжність відокремлено від руху, створення чи видалення платформ.
- Непряме керування Walker відокремлено від прямого аватара й розміщення
  напрямних команд.

## Український підсумок

Echochrome не дає прямої команди «йди ліворуч» чи «перестрибни сюди». Walker
рухається сам, а гравець повертає камеру навколо незмінного 3D-лабіринту. Якщо
два далекі кінці доріжок збігаються на екрані, гра дозволяє перехід. Якщо ближча
...
- 2026-08-14: selected as `GAME-0092` to falsify reuse of Fez's cardinal
  projection slice under continuous camera control and autonomous locomotion.
- Bounded Perspective Travelling and Perspective Existence plus the all-echo
  Solo-stage objective; excluded the other three laws and non-Solo modes.
- Used official Sony and PlayStation records to separate fixed 3D geometry,
  live projection transformation and indirect Walker guidance.
- Added an executable two-echo control with five sampled projections and six
  rejected invalid transitions.
