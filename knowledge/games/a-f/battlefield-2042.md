...
- [Confirmed | Corroborated | High] Поточний звичайний Conquest об'єднує
  окремі точки в сектори, дає сектор лише за всі його точки й перетворює
  більшість секторів на тиск на спільні підкріплення (`BF2042-003`–`BF2042-005`).
- [Confirmed | Direct | High] Plus змінює сумісне оснащення чинної зброї під
  живим керуванням, а call-in створює доступну наземну машину у вибраній
  допустимій позиції за спільної місткості й cooldown (`BF2042-008`,
  `BF2042-009`).

## Нові гени

- [Confirmed | Corroborated | High] `ACT-403`, `ACT-404`, `SYS-742`,
  `SYS-743`, `SYS-744`, `SYS-745`, `CON-574`, `CON-575`, `CON-576`,
  `INF-283`, `INF-284`, `INF-285`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0232`.

## Зміни таксономії

- [Observation | Direct | High] Окремого taxonomy-change немає; попередні
  сигнатури й lifecycle не змінено.

## Нові питання

- Чи повторює інший Conquest-подібний ruleset одночасно all-point sectors,
  majority-ticket pressure, live weapon reconfiguration і player-positioned
  shared vehicle delivery?

## Наступна рекомендована одиниця

- [Confirmed | Direct | High] `SEARCH_DEMAND_BATCH_011_AUDIT`.
- Optimisation criterion: незалежно перевірити всі дев'ять локальних game units,
  фінальні 234-game totals, scope, джерела, переходи, сигнатури, переклади,
  artwork, indexes, generated artifacts і browser/accessibility parity.
- Backlog impact: завершити останню game unit цього Goal, не починаючи audit у
  цьому turn.

## Чому саме вона

- [Confirmed | Direct | High] Це записана Unit 10 і фінальна acceptance boundary
  пакета після `GAME-0226`–`GAME-0234`.
