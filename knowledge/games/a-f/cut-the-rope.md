...
- [Confirmed | Corroborated | High] A swipe changes rope topology at one
  moment, after which deterministic gravity, tension and momentum keep changing
  the candy state before another command (`CTR-003`–`CTR-006`).
- [Observation | Corroborated | High] Mandatory Om Nom delivery and optional
  contact-star score are distinct evaluation layers (`CTR-007`, `CTR-008`,
  `CTR-012`).

## Нові гени

- [Observation | Corroborated | High] Added `ACT-027`, `SYS-036`, `SYS-037`,
  `CON-060`, `CON-061` and `OBJ-014`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0021` captures irreversible
  support release into continuously evolving payload delivery.

## Зміни таксономії

- [Observation | Corroborated | High] Змін таксономії немає; continuous
  physics separates cleanly into System Behaviour plus existing Time.

## Нові питання

- Does World of Goo reuse `SYS-036` while replacing destructive support
  removal with additive load-bearing construction?
- Does a non-physics delivery puzzle support `OBJ-014` without sharing the
  continuous-time genes?

## Наступна рекомендована гра

- [Hypothesis | Limited | High] `GAME-0022` — Opus Magnum.
- Optimisation criterion: maximise mechanical distance after a live-physics
  game while testing construction / execution and multi-metric evaluation.
- Expected information gain: distinguish player-built program state from its
  repeated automatic execution and test whether the existing Time types cover
  explicit design-then-run cycles.
- Backlog impact: remove Opus Magnum from the retained pool; preserve Lemmings,
  Return of the Obra Dinn, Gorogoa and World of Goo.

## Чому саме вона

- [Hypothesis | Limited | High] It moves away from continuous intervention to
  self-paced machine design followed by cyclic execution, probing an uncovered
  command and scheduling structure with strong developer documentation.
