...
- Added one reviewed game, one strict-subset combination and five transferable
  genes for authority-backed living arrest, maintained hostile compliance,
  custody legality, investigation/evaluation information and episode success.
- Reused twenty-nine lower-ID genes without changing any earlier signature.
- Extended the corpus from 242 to 243 games and from 240 to 241 combinations.

## Нові факти

- Актуальний Windows-продукт лишається доступним, попри окремо оголошене
  припинення консольних сервісів; Steam продає право Ultimate Edition, а не
  нібито поточну Standard Edition.
- Episode 1 поєднує сканування, гільзу-відволікання, ордер, докази, покору,
  живий арешт і обов’язковий фінальний бій у одному відтворюваному пакеті.

## Нові гени

- `ACT-417`, `SYS-766`, `CON-587`, `INF-293`, `OBJ-151`.
- Їхні назви описують переносні дії, стани, обмеження, інформацію й термінал;
  `Back to School`, імена підозрюваних та конкретні очки лишилися параметрами.

## Нові комбінації

- `COMB-0241`: сканування й позначення, інертне відволікання, читана
  настороженість, підтримувана загрозою покора, жива варта, докази та
  оцінений термінал епізоду.

## Зміни таксономії

- Нових перейменувань, злиттів, депрекацій або змін сигнатур раніше reviewed
  ігор немає. Додано п’ять нових Active-генів і підтримку чотирьох наявних
  родин: `FAM-009`, `FAM-010`, `FAM-012`, `FAM-017`.

## Similarity notes

- Genre similarity to Battlefield multiplayer is weak evidence here: the
  decision identity comes from campaign-only Scanner and `Freeze` verbs.
- Far Cry 3 supplies the strongest tested comparison corridor but cannot
  explain living custody, warrant scoring or Episode settlement.
- The Witcher 3 shares authored evidence conversion and quest progression but
  its evidence prepares a monster hunt; it has no tactical target marks,
  hostile compliance or arrest coverage.

## Experience hypotheses

- Players who like pre-contact reconnaissance and reversible stealth plans may
  value the shift from lethal clearance to controlled living custody.
- The strongest tension should arise when maintaining aim over several
  compliant suspects competes with movement, partner position and unseen risk.
- Evaluation may teach more clearly than simple clearance because an escaped
  living-arrest opportunity can reduce Expert outcome without falsely ending
  the episode.

## New questions

- Which other reviewed games make continuing aim coverage, rather than a
  one-time command, the resource that preserves a surrender state?
- Does a later Hardline episode preserve the same custody grammar while changing
  evidence order enough to justify a separate combination?

## Next recommended unit

- [Hypothesis | Corroborated | High] `SEARCH_DEMAND_BATCH_012_AUDIT`.
- Independently verify `GAME-0235`–`GAME-0243`, final totals, sources, scopes,
  transitions, taxonomy labels, Ukrainian parity, artwork and web acceptance.

## Research artefacts

- `research/taxonomy-health/BASELINE_243.json`.
- `research/normalisation/GENE_DUPLICATE_QUEUE_093.md` and `.json`.
- `research/normalisation/GENE_LEXICAL_TOP_N_EXPERIMENT_093.md` and `.json`.

## Evidence gaps

- EA exposes no public immutable PC patch label for the current Steam package;
  Build `10351826` is therefore a secondary distribution observation.
- The official manual defines the transferable rules but not every Episode 1
  encounter order; that route is corroborated by one maintained textual guide.
- Direct play and audiovisual evidence were not used, so exact score values,
  positions and optional encounter branches remain parameters rather than
  canonical claims.

## Confidence

High for lawful current PC availability, product/mode separation, controls,
scanner, warrants, compliance, arrest and Episode interface; medium-high for
the exact Build ID and Episode 1 route/terminal reconstruction.
