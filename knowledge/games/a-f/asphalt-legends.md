...
- [Confirmed | Direct/Corroborated | High] Актуальний продукт, постійний
  Career, Classic, TouchDrive, Flags і nitro визначені `ASPH-001`–`ASPH-010`;
  збірка та точні параметри Race 1 мають середню впевненість.

## Нові гени

- [Confirmed | Direct/Corroborated | High] П’ять нових переносних генів
  відокремлюють підказаний маневр, системне ведення траєкторією, наповнення
  nitro, інформацію TouchDrive і прийнятний діапазон місць.

## Нові комбінації

- [Confirmed | Corroborated | High] `COMB-0240` поєднує допоміжне ведення,
  підказані рішення, nitro, суперників і збережений пороговий результат.

## Зміни таксономії

- Раніше reviewed підписи, визначення й lifecycle не змінено.

## Нові питання

- Які інші reviewed arcade racers зберігають гілкові рішення, коли система
  забирає безперервне кермування?
- Як Manual Drive змінює сигнатуру тієї самої Career-гонки?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0243` — Battlefield Hardline.
- Порівняти коротку сюжетну поліцейську місію з транспортною гонкою, не
  змішуючи campaign і multiplayer.

## Research artefacts

- `research/taxonomy-health/BASELINE_242.json`.
- `research/normalisation/GENE_DUPLICATE_QUEUE_092.md` and `.json`.
- `research/normalisation/GENE_LEXICAL_TOP_N_EXPERIMENT_092.md` and `.json`.

## Evidence gaps

- Gameloft has no static official page naming every current Race 1 parameter;
  build, starter, route, rank, exact rewards and threshold are corroborated
  observations. TouchDrive's steering boundary is official; prompt details are
  reconstructed from current written descriptions.
- No direct play and no audiovisual evidence were used.

## Confidence

High for product, Career/Classic grammar, assisted-control boundary, nitro and
Flag settlement; medium-high for exact first-race parameters and transitions.
