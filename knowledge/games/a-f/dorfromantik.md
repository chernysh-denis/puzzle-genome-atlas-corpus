...
- [Confirmed | Direct / Corroborated | High] Classic mode consumes mandatory
  procedurally supplied tiles, but perfect placements and quests can add tiles
  back before stack exhaustion (`DOR-001`, `DOR-006`–`DOR-009`).
- [Observation | Corroborated | High] Dorfromantik couples expanding spatial
  commitment with a replenishable action horizon rather than a fixed board or
  strictly decreasing move counter (`DOR-009`, `DOR-010`).

## Нові гени

- [Observation | Corroborated | High] The original analysis added `ACT-026`,
  `SYS-034`, `SYS-035`, `CON-056`, `CON-057`, `CON-058` and `CON-059`;
  `CON-057` was later merged into `CON-039` by `TAXONOMY_CHANGE_002`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0020` captures mandatory rotatable
  frontier placement whose edge/group quality earns replacement supply.

## Зміни таксономії

- [Observation | Corroborated | High] `TAXONOMY_CHANGE_002` merges the
  game-specific supply-order constraint into shared `CON-039`; the six-type
  model is unchanged.

## Нові питання

- Does the replenishable-supply boundary recur in a non-spatial puzzle, or is
  current evidence still coupled too closely to placement quality?
- Does a third previewed-supply game preserve the `SYS-024` boundary when its
  underlying supply is deterministic rather than procedurally generated?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `CHECKPOINT_020` — scheduled 20-game corpus audit.
- Optimisation criterion: audit reuse, family coverage, source quality and
  singleton pressure before selecting `GAME-0021`.
- Expected information gain: determine whether seven Dorfromantik additions
  clarify genuine gaps or indicate over-fine placement-specific boundaries.
- Backlog impact: refresh the empty coverage pool from sourced candidates.

## Чому саме вона

- [Confirmed | Direct | High] The research plan requires a checkpoint after
  the twentieth reviewed game, before another subject is selected.
