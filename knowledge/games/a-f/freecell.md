...
- [Confirmed | Corroborated | High] FreeCell separates known-card access from
  hidden information: every card is visible, but only stack ends are directly
  transferable (`FRC-001`, `FRC-002`, `FRC-007`).
- [Confirmed | Corroborated | High] Free cells and empty cascades determine
  which ordered sequences can be rearranged (`FRC-003`, `FRC-005`).
- [Pattern | Corroborated | Medium] Tableau construction preserves transport
  flexibility, while foundation construction irreversibly advances the target
  order (`FRC-004`, `FRC-008`).

## Нові гени

- [Observation | Corroborated | High] `ACT-010`, `CON-014`, `CON-015`,
  `CON-016` and `CON-017`.
- [Observation | Corroborated | High] `INF-001`, `OBJ-004` and `TIM-002` are
  reused.

## Нові комбінації

- [Confirmed | Corroborated | High] `COMB-0007` — ordered card reconstruction
  through bounded temporary buffers.

## Зміни таксономії

- [Observation | Corroborated | Medium] Змін таксономії немає. The six types
  represent open information, access restrictions and ordered completion.

## Нові питання

- TODO: compare a concealed-card solitaire to test `CON-014` against
  `INF-003` and reveal behaviours.
- TODO: test `CON-015` in a non-card sorting puzzle with bounded temporary
  vessels or slots.
- TODO: compare a variant whose group move is primitive and not decomposable
  into legal single-element moves.

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0008` — Nonogram.
- Optimisation criterion: test exact aggregate clues over overlapping line
  variables after a transport-and-buffer puzzle.
- Expected information gain: distinguish row/column run specifications from
  Minesweeper's local hazard counts and Sudoku's all-different units, while
  testing completion under binary assignment.
- Backlog impact: Nonogram moves from the coverage pool to the immediate task;
  FreeCell leaves the pool after completion.

## Чому саме вона

- [Hypothesis | Limited | Medium] Nonogram returns to static deduction but
  introduces ordered run-length clues rather than local counts or uniqueness.
- It completes the eight-game diversity sample needed for the first taxonomy
  checkpoint without following FreeCell with another transport puzzle.
