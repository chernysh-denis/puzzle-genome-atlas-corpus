...
- [Confirmed | Corroborated | High] У стандартному Black Box кожен крайовий
  промінь детерміновано дає удар, відбиття або парний вихід, не показуючи
  внутрішньої траєкторії (`BLB-001`–`BLB-005`).
- [Confirmed | Corroborated | High] Правильність визначає повна спостережна
  еквівалентність, а не обов’язкова тотожність прихованих координат
  (`BLB-007`, `BLB-008`).

## Нові гени

- [Observation | Corroborated | High] Додано `ACT-074`, `ACT-075`, `SYS-105`,
  `SYS-106`, `CON-111`, `INF-032` і `OBJ-042`; повторно використано `ACT-007`,
  `INF-003` і `TIM-002`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0066` — відновлення прихованого
  простору крайовими променями до повної спостережної еквівалентності.

## Зміни таксономії

- [Observation | Corroborated | High] Межі шести типів збережено; реєстр
  розширено сімома операційно відмінними генами.

## Нові питання

- Чи повторно використає Wordle повне подання гіпотези, але потребуватиме
  позиційного категорійного відгуку замість Black Box-проб і Mastermind-лічби?
- Чи Simon відокремить приховану симуляцію без зовнішнього зондування від
  `SYS-105` і перевірки спостережної еквівалентності?

## Наступна рекомендована гра

- [Hypothesis | Limited | High] Simon.
- Optimisation criterion: checkpoint-ranked information gain after Black Box.
- Expected information gain: hidden simulation observed through experiments
  can challenge the new probe / propagation split without reusing perimeter
  geometry.
- Backlog impact: retain Wordle and The Room as later hidden-information
  controls; rescore the pool after Simon.

## Чому саме вона

- [Hypothesis | Limited | High] Simon was second in the 65-game checkpoint at
  `12 / 15`; after Black Box it is the clearest independent falsifier of
  observation generated by a concealed system rather than direct state reveal.
