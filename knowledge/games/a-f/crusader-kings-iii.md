...
- [Confirmed | Corroborated | High] `CK3-001`–`CK3-012`: one current base-game
  Murchad tutorial links character, marriage, council, titles, claims, levies,
  battle, siege and war settlement to an explicit completion state.

## Нові гени

- [Observation | Corroborated | High] Added `ACT-365`–`ACT-369`,
  `SYS-670`–`SYS-679`, `CON-538`–`CON-543`, `INF-260`–`INF-264` and `OBJ-126`.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0202` — character, office and title
  dependencies converted through a claim-bounded tutorial war into settlement.
...
- Which current Witcher 3 base-game White Orchard contract has one retained
  quest-complete terminal without importing expansions or the whole prologue?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0205` — The Witcher 3: Wild Hunt.
- Optimisation criterion: test authored investigation, preparation and combat
  after one actor-network grand-strategy tutorial.
- Expected information gain: move from open simulation constrained by a guided
  terminal to one retained open-world contract result.
- Backlog impact: continue Selection 008 only after current base-game build,
  difficulty and White Orchard quest terminal are fixed.

## Чому саме вона

- [Hypothesis | Limited | Medium] The Witcher 3 should add clue-led contract
  preparation and monster combat while preserving an explicit retained quest
  completion boundary.
