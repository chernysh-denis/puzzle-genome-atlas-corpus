...
- Поточний Lineage II Live відокремлено від Classic, Aden, Wolf і Samurai та
  зафіксовано на межі 1 вересня 2026 року.
- `Path of Destiny - Beginning` утримує перший маршрут не-Ertheia рівнів 1–20;
  одиниця фіксує Human Fighter, Chronos і Warrior.

## Нові гени

- `ACT-387`, `SYS-712`, `CON-559` і `OBJ-136` ізолюють вибір, застосування,
  умови та завершення першого постійного класового переходу.

## Нові комбінації

- `COMB-0217` — від керованого бою й досвіду через класовий квест до постійної
  першої зміни класу.

## Зміни таксономії

- Додано чотири Active-межі без зміни наявних генів або сигнатур.

## Нові питання

- Чи дасть World of Warcraft іншу початкову структуру, якщо клас фіксується до
  першого квесту, а не здобувається як його термінал?

## Наступна рекомендована гра

- [Confirmed | Direct | High] `GAME-0220` Battlefield V.
- Optimisation criterion: compare a current fixed multiplayer packet with the
  persistent MMORPG route while preserving Batch 010 order.
- Expected information gain: one bounded class-and-objective team match.
...
- It is the next authorised subject in `SEARCH_DEMAND_GAME_SELECTION_010` and
  keeps the requested MMORPG series separated by contrasting units.
