...
- [Confirmed | Corroborated | High] Дев'ять звичайних лунок World 1 поєднують
  видиму скінченну руку одноразових карт із фіксованими схилами
  (`GPK-001`-`GPK-010`).

## Нові гени

- [Observation | Corroborated | High] `SYS-100` - покрокове проходження м'яча
  висотним полем за параметрами карти.
- [Observation | Corroborated | High] `CON-103` - скінченна невідновлювана рука
  просторових команд.
- [Observation | Corroborated | High] `OBJ-014` - доставити визначений payload
  у фіксований receiver; тут це керований картами м'яч і лунка.

## Нові комбінації

- [Confirmed | Corroborated | High] `COMB-0057` - скінченне впорядкування
  одноразових карт для точного проходження м'яча висотним полем.

## Зміни таксономії

- [Observation | Corroborated | High] `ACT-061` узагальнено так, щоб допустимим
  просторовим параметром цілі міг бути напрямок; normalisation 004 пізніше
  об'єднала `OBJ-036` із `OBJ-014`.

## Нові питання

- Чи повторюється `CON-103` у детермінованій не картковій системі з вибірковим
  споживанням просторових команд?
- Чи потребуватимуть повітряні та складені карти пізніх світів окремої системної
  межі, чи залишаться параметрами `SYS-100`?

## Наступна рекомендована гра

- [Hypothesis | Corroborated | High] inbento.
- Optimisation criterion: test fixed recipe reconstruction and layered tile
  manipulation without repeating card-command traversal.
- Expected information gain: separate finite piece manipulation, overlap and
  exact target-pattern display from Golf Peaks' consumable command hand.
- Backlog impact: preserves broader word and continuous-simulation candidates
  while avoiding another routing game.

## Чому саме вона

- [Hypothesis | Corroborated | High] inbento is mechanically distant despite
  sharing a developer: it replaces trajectory commands with visible finite
  food-piece composition and can test whether existing configuration genes
  survive a new layered-packing family.
