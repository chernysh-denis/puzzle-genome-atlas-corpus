...
- [Confirmed | Direct | High] Original app `70`, the definitive public branch,
  `Medium`, follow/wait, charger, flashlight, breakage and save rules are fixed
  by `HL1-001`–`HL1-003` and `HL1-005`–`HL1-012`.
- [Observation | Corroborated | High] The ruined-chamber entry, seven-map route,
  scientist scanner dependency and first `Office Complex` terminal are fixed by
  `HL1-004`, `HL1-007`, `HL1-012` and `HL1-013`.

## Нові гени

- [Observation/Confirmed | Direct | High] `ACT-408`, `ACT-409`, `SYS-752`–
  `SYS-755` and `CON-580` isolate portable ally, station, illumination and
  breakable-object boundaries without importing Half-Life names or numbers.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0237` joins a transient role-capable
  ally, finite continuous restoration, rechargeable local light, damage-opened
  topology, live combat and one retained chapter successor.
...
- Does another authored first-person route require a freely toggled world ally
  to perform a role-specific gate without turning that ally into party inventory?
- Which later game reuses both finite held-use restoration and automatically
  recharging portable light while preserving a chapter-level terminal?

## Наступна рекомендована гра

- [Hypothesis | Limited | Medium] `GAME-0240` — Kingdom Come: Deliverance II.
- Optimisation criterion: compare a current evidence-rich first-person authored
  route that replaces science-facility mechanisms with grounded skill,
  dialogue, equipment and quest-state constraints under one retained terminal.

## Research artefacts

- `research/taxonomy-health/BASELINE_239.json`.
- `research/normalisation/GENE_DUPLICATE_QUEUE_089.md` and `.json`.
- `research/normalisation/GENE_LEXICAL_TOP_N_EXPERIMENT_089.md` and `.json`.

## Evidence gaps

- Public Build ID is secondary SteamDB metadata; Valve's 2024 update and 25th
  Anniversary page independently establish the customer-facing branch but do
  not print that Build ID.
- No direct play was conducted, so exact run-specific stock, damage, actor
  survival, autosave placement and timing remain parameters rather than facts.
- The preserved Day One manual is an original rules document but not a current
  official host; Valve SDK source independently corroborates the admitted
  shared systems.
- No audiovisual evidence was used. Route claims depend on static written
  sources and are limited to the declared chapter transition.

## Confidence

High for product, branch, difficulty, interaction, resource, combat, save and
chapter-order boundaries. Medium-high for the exact evidence-reconstructed
chapter trace because no direct play was conducted.
