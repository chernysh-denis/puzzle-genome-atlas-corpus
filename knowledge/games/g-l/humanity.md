...
- [Confirmed | Direct | High] HUMANITY stores commands in the world and reuses
  them for every later walker, rather than assigning a skill to an individual
  (`HUM-002`, `HUM-003`, `HUM-007`).
- [Confirmed | Direct | High] A recurring population can reuse `OBJ-019` because
  completion still requires a minimum credited flow through a fixed goal
  (`HUM-004`–`HUM-006`).

## Нові гени

- [Observation | Corroborated | High] Added `ACT-042`, `SYS-055` and `CON-075`;
  nine existing genes are reused.

## Нові комбінації

- [Observation | Corroborated | High] `COMB-0029` captures avatar-local
  persistent command programming of an autonomous rescue stream.

## Зміни таксономії

- [Observation | Corroborated | High] Змін таксономії немає; `SYS-047` and
  `OBJ-019` gain recurring-population parameters without changing identity.

## Нові питання

- Does the excluded Start Switch variant instantiate `TIM-006` exactly, or a
  narrower one-shot command-field planning phase?
- Can the six-gene shared Lemmings / HUMANITY rescue motif justify a future
  higher-level combination record without replacing either verified subset?

## Наступна рекомендована гра

- [Hypothesis | Limited | High] `GAME-0030` — Tin Hearts.
- Optimisation criterion: perform the closer falsification test after HUMANITY
  rejected full `COMB-0025` support.
- Expected information gain: test whether movable physical routing devices,
  rewind and a finite tin-soldier quota reproduce or further split Lemmings'
  role-assignment grammar.
- Backlog impact: rebuild the coverage pool after the sourced retained set is
  exhausted.

## Чому саме вона

- [Hypothesis | Limited | High] Publisher documentation explicitly exposes
  forward walking, obstacle routing, pause / fast-forward / rewind and rescue
  counts, making Tin Hearts the strongest available audit of which Lemmings
  genes are truly reusable.
