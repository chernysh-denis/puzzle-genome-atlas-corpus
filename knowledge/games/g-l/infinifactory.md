...
- [Confirmed | Corroborated | High] Training Routine 1 separates self-paced 3D
  conveyor layout from a repeated exact-output factory run (`INFY-001`–
  `INFY-007`).
- [Observation | Corroborated | High] It shares eight genes with Opus Magnum
  while falsifying symbolic-program and kinematic-halt inheritance
  (`INFY-008`, `INFY-012`).

## Нові гени

- [Observation | Corroborated | High] Added `SYS-077` for discrete contact-
  driven conveyor transport; reused eight existing genes.

## Нові комбінації

- [Confirmed | Corroborated | High] Registered recurring `COMB-0042`, supported
  by Opus Magnum and Infinifactory.

## Зміни таксономії

- [Observation | Corroborated | High] Змін таксономії немає; three descriptions
  were made representation-neutral without changing boundaries or signatures.

## Нові питання

- Does a later Infinifactory welder / sensor scope instantiate passive
  geometry-triggered transformation, or require a distinct signal-network
  control boundary?
- Will another passive factory game reuse `SYS-077` without inheriting exact
  structured output or separate edit-run time?

## Наступна рекомендована гра

- [Hypothesis | Corroborated | High] `GAME-0043` — Stephen's Sausage Roll.
- Optimisation criterion: move from machine programming to a mechanically
  independent push / roll system and test older Action / Constraint singletons.
- Expected information gain: distinguish body-mediated push, rigid two-cell
  roll, fork spearing, surface-state cooking and exact-once completion.
- Backlog impact: remove Stephen's Sausage Roll from the retained pool; keep A
  Good Snowman Is Hard to Build, Snakebird and Hexcells Infinite.

## Чому саме вона

- [Hypothesis | Corroborated | High] It ranked second in targeted selection 005
  and supplies the strongest independent Action / Constraint falsifier after a
  machine-family analysis, without topology or rewind repetition.

## Sources consulted

- Zachtronics' product page and developer-published Steam listing.
- Contemporary PC Gamer and PCGamesN accounts and the detailed Gold-Plated
  Games review.
- Bounded specialist level / score artefacts and two community rule checks for
  Training Routine 1 metrics, conveyor-only scope and stall semantics.
